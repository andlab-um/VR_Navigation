...
let expName = 'VR_Memory_Test_Block';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  expInfo: expInfo,
  resources: [
    {'name': 'bac.png', 'path': 'bac.png'},
    {'name': 'TestM2.PNG', 'path': 'TestM2.PNG'}
  ]
});
...
async function experimentInit() {
  ...
  image = new visual.ImageStim({
    ...
    image : 'TestM2.PNG', mask : undefined,
  ...
  w_file = (("orderexcel/" + excelnumber.toString()) + ".xlsx");
  ...
  import {StreamInfo, StreamOutlet} from 'pylsl';
  var info, outlet;
  info = new StreamInfo("EEGStream", "Markers", 1, 0, "string", "marker");
  ...
  trialnClock = new util.Clock();
  bac = new visual.ImageStim({
    win : psychoJS.window,
    name : 'bac', units : undefined, 
    image : 'bac.png', mask : undefined,
    ori : 0.0, pos : [0, 0], size : [1.76, 0.99],
    color : new util.Color([1,1,1]), opacity : undefined,
    flipHoriz : false, flipVert : false,
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisTrial of trials) {
      ...
      trialsLoopScheduler.add(trialnRoutineBegin(snapshot));
      trialsLoopScheduler.add(trialnRoutineEachFrame());
      trialsLoopScheduler.add(trialnRoutineEnd(snapshot));
...
function trialnRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'trialn' ---
    t = 0;
    trialnClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    ...
    psychoJS.experiment.addData("stage", "Recall");
    ...
    // keep track of which components have finished
    trialnComponents = [];
    trialnComponents.push(bac);
    trialnComponents.push(image_4);
    trialnComponents.push(key_resp_4);
    
    for (const thisComponent of trialnComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function trialnRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'trialn' ---
    // get current time
    t = trialnClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    
    // *bac* updates
    if (t >= 0.0 && bac.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      bac.tStart = t;  // (not accounting for frame time here)
    ...
    if (bac.status === PsychoJS.Status.STARTED && t >= frameRemains) {
    ...
    if (key_resp_4.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_4.getKeys({keyList: ['s', 'k'], waitRelease: false});
...
function trialnRoutineEnd(snapshot) {
  return async function () {
