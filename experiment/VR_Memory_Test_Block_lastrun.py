...
expName = 'VR_Memory_Test_Block'  # from the Builder filename that created this script
expInfo = {
    'participant': f"{randint(0, 999999):06.0f}",
    'age': '0',
    'gender': 'please input M/F',
    'version': 'ESY',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Navigation_VR\\Social_Navigation_VR\\VR_Memory_Test_Block_lastrun.py',
...
# Run 'Begin Experiment' code from startup_code_2
...
if expInfo['version'] == 'ESY':
    w_file_whole = 'wfile_ESY.xlsx'
    arrange_file = 'Arrange_ESY.xlsx'
else:
    w_file_whole = 'wfile.xlsx'
    arrange_file = 'Arrange.xlsx'
# Run 'Begin Experiment' code from lsl_init_2
...
text = visual.TextStim(win=win, name='text',
    text='',
    font='Open Sans',
    pos=(0.65, 0.4), height=0.2, wrapWidth=None, ori=0.0, 
...
slider = visual.Slider(win=win, name='slider',
    startValue=None, size=(0.8, 0.08), pos=(0, -0.1), units=None,
...
text_2 = visual.TextStim(win=win, name='text_2',
    text='评估一下你在VR眼镜场景物体记忆任务和判断物体是否在场景的任务中选择的表现，\n请你对自己对场景的记忆水平打分（最低1分，最高7分）',
    font='Arial',
    pos=(0, 0.2), height=0.04, wrapWidth=2.0, ori=0.0, 
...
trials_2 = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions(w_file_whole),
...
for thisTrial_2 in trials_2:
    ...
    key_resp.rt = []
    _key_resp_allKeys = []
    text.setText((trials_2.thisN+1))
    # keep track of which components have finished
    introComponents = [key_resp, image, text]
    ...
    for thisTrial in trials:
        ...
        thisExp.addData("stage","Recall")
        
        if indicator == 'scene_things':
            valuex = "15"
            
        elif indicator == 'building':
            valuex = "16"
        ...
        outlet.push_sample([valuex+valuey])
        # keep track of which components have finished
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if bac.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > bac.tStartRefresh + 20-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if image_4.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_4.tStartRefresh + 20-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_resp_4.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_resp_4.tStartRefresh + 20-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        outlet.push_sample([valuex+valuez])
        print(valuex+valuez)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
    ...
    # keep track of which components have finished
    rateComponents = [slider, status_mouse, vtxt, text_2]
    ...
    outlet.push_sample(["199"])
    # store data for trials_2 (TrialHandler)
    trials_2.addData('status_mouse.x', status_mouse.x)
    trials_2.addData('status_mouse.y', status_mouse.y)
    trials_2.addData('status_mouse.leftButton', status_mouse.leftButton)
    trials_2.addData('status_mouse.midButton', status_mouse.midButton)
    trials_2.addData('status_mouse.rightButton', status_mouse.rightButton)
    trials_2.addData('status_mouse.time', status_mouse.time)
...
for thisTrial_3 in trials_3:
    ...
    scenes = [label1, label2, label3, label4, label5, label6,label7, label8,label9,label10,label11,label12,label13,label14,startpoint, endpoint]
    scene_value = ['flower','bin','sofa','plant','mailbox','wash','CaseAvatar1', 'CaseAvatar2', 'CaseAvatar3', 'CaseAvatar4', 'CaseAvatar5', 'CaseAvatar6', 'CaseAvatar7', 'CaseAvatar8','startpoint','endpoint']
    ...
    #    if new_scene_value[pointer] != "startpoint" and new_scene_value[pointer] != "endpoint":
    #        new_scenes[pointer].setSize([new_size, new_size])
    ...
    label14.setImage(t8)
    # keep track of which components have finished
    trialpComponents = [image_3, mouse, scene, label1, label2, label3, label4, label5, label6, label7, label8, label9, label10, button, label11, label12, label13, label14, startpoint, endpoint]
        ...
        if button.status == STARTED:
            ...
            if button.isClicked:
                ...
                if not button.wasClicked:
                    ...
                    screenshot.save('data/'+expInfo['participant']+'_'+expInfo['date']+'_arrangement.png') 
    ...
    #thisExp.nextEntry()
    outlet.push_sample(["204"])
    print(204)
    screenshot = win.getMovieFrame()
    screenshot.save('data/'+expInfo['participant']+'_'+str(pictype)+'_arrangement.png') 
    win.clearBuffer()
    trials_3.addData('button.numClicks', button.numClicks)
    if button.numClicks:
       trials_3.addData('button.timesOn', button.timesOn)
       trials_3.addData('button.timesOff', button.timesOff)
    else:
       trials_3.addData('button.timesOn', "")
       trials_3.addData('button.timesOff', "")
